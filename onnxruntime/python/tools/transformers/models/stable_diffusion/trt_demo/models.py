    ...
    def get_model(self, framework_model_dir):
        ...
        if not os.path.exists(clip_model_dir):
            model = CLIPTextModel.from_pretrained(
                self.path, subfolder=self.subfolder, use_safetensors=self.hf_safetensor, use_auth_token=self.hf_token
            ).to(self.device)
...
def make_CLIP(
    version, pipeline, hf_token, device, verbose, max_batch_size, output_hidden_states=False, subfolder="text_encoder"
):
    return CLIP(
        version,
        pipeline,
...
def make_CLIPWithProj(
    version, pipeline, hf_token, device, verbose, max_batch_size, subfolder="text_encoder_2", output_hidden_states=False
):
    return CLIPWithProj(
        version,
        pipeline,
    ...
    def get_sample_input(self, batch_size, image_height, image_width):
        ...
        if self.controlnet is None:
            ...
            return (
                ...
                torch.randn(batch_size, self.text_maxlen, self.embedding_dim, dtype=dtype, device=self.device),
                torch.randn(
                    len(self.controlnet), batch_size, 3, image_height, image_width, dtype=dtype, device=self.device
                ),
...
def make_UNetXL(version, pipeline, hf_token, device, verbose, max_batch_size):
    ...
    return UNetXL(
        ...
        max_batch_size=max_batch_size,
        unet_dim=4,
        time_dim=(5 if pipeline.is_sd_xl_refiner() else 6),
    )
...
class VAE(BaseModel):
    def __init__(
        ...
        super().__init__(version, pipeline, hf_token, device=device, verbose=verbose, max_batch_size=max_batch_size)
    ...
    def get_model(self, framework_model_dir):
        ...
        if not os.path.exists(vae_decoder_model_path):
            vae = AutoencoderKL.from_pretrained(
                self.path, subfolder="vae", use_safetensors=self.hf_safetensor, use_auth_token=self.hf_token
            ).to(self.device)
    ...
    def __init__(self, version, pipeline, hf_token, device, path, framework_model_dir, hf_safetensor=False):
        ...
        if not os.path.exists(vae_encoder_model_dir):
            self.vae_encoder = AutoencoderKL.from_pretrained(
                path, subfolder="vae", use_safetensors=hf_safetensor, use_auth_token=hf_token
            ).to(device)
